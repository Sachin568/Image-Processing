...
%%% but this whole loop is useless 
for s = 1:nscale,                                                           % For each scale. 
    
    % Construct the filter - first calculate the radial filter component.
    fo = 1.0/wavelength;                                                    % Centre frequency of filter.
    rfo = fo*2;                                                             % Normalised radius from centre of frequency plane 
    
    % corresponding to fo.
    logGabor(1:ndata/2+1) = exp((-(log(radius/fo)).^2) / (2 * log(sigmaOnf)^2));  %make it an integer
    logGabor(1) = 0;  
    filter = logGabor;
...
%imshow(filtersum);title('filtersum'); %display(EO{s});